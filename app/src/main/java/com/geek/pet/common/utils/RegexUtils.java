...
public class RegexUtils {
    ...
    /**
     * 验证密码
     * <p>取值范围为含有小写字母、大写字母、数字、特殊符号的两种及以上,必须是6-16位</p>
     *
     * @param input 待验证文本
     * @return {@code true}: 匹配<br>{@code false}: 不匹配
     */
    public static boolean isPassword(CharSequence input) {
        return isMatch(ConstUtils.REGEX_PASSWORD, input);
    }

    /**
     * 验证密码
     * <p>取值范围为含有小写字母、大写字母、数字的两种及以上,必须是6-12位</p>
     *
     * @param input 待验证文本
     * @return {@code true}: 匹配<br>{@code false}: 不匹配
     */
    public static boolean isPasswordSimple(CharSequence input) {
        return isMatch(ConstUtils.REGEX_PASSWORD_SIMPLE, input);
    }
